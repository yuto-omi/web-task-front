...
import { type Task, type TaskPriority, type TaskStatus, updateTask } from '@/features/tasks/api';
...
export function ProjectTaskEditModal({ task, phases, members, open, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-sm font-medium mb-4">タスクを編集</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
          ...
          {/* ステータス */}
          <div>
            <label className="text-xs text-gray-600 font-medium block mb-1">ステータス</label>
            <select
              {...register('status')}
              className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-100 bg-white"
            >
